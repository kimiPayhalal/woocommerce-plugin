 ...
 * Author:  Soqua Fintech Sdn Bhd
 * Author URI: https://payhalal.net
 * Version: 1.0.1
 *
	...
	function payhalal_init_gateway_class() {
		...
		class payhalal_Gateway extends WC_Payment_Gateway {
		 	...
		 	public function request_handler() {
		 		...
		 		if($order_id > 0){
			 		...
			 		if($order != ""){
						...
						$data_out["customer_phone"] = $user_info->user_phone;
						$data_out["language"] = "en";
						$data_out["hash"] =  self::ph_sha256($data_out, $this->private_key);
			...
			public function ph_sha256( $data, $secret ) {
				...
				$hash =  hash('sha256',$secret.$data["amount"].$data["currency"].$data["product_description"].$data["order_id"].$data["customer_name"].$data["customer_email"].$data["status"]);
				return $hash;
			}
